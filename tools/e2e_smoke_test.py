...
IMPORTANTE — cosas que ya rompieron este script antes y por qué:
  ...
  2. CityMapScene, LocationScene y DialogueScene comparten un mismo
     frame: columna izquierda = arte arriba + texto abajo, columna
     derecha = lista de acciones contextuales (viajar, hablar, explorar,
     o las opciones de un diálogo). Las coordenadas viven centralizadas
     en tools/frame_coords.py (`action_menu_item(i)` / `dialogue_option(i)`
     / `icon_toolbar_item(i)`) — actualizar ESE archivo si el layout
     vuelve a cambiar, no cada script suelto. SuspectBoardScene/
     CrimeComputerScene/DebugScene/EndingScene NO forman parte de ese
     frame y siguen con coordenadas propias hardcodeadas acá.
  3. El ÍNDICE de cada ítem de la lista depende de CUÁNTOS ítems lo
     ...
     comenta explícitamente qué índice corresponde a qué acción y por
     qué, basado en `src/data/locations.ts` (npcIds por lugar) y
     `src/data/zoneConnections.ts` (conexiones por zona).
  4. FASE 24 — comparando contra capturas reales del juego de referencia:
     "Ver el mapa", "Pizarrón", "Expediente" y "Sistema de Inteligencia
     Criminal" SALIERON de esta lista — ahora son 4 íconos al pie de la
     misma columna (`icon_toolbar_item('mapa'|'pizarron'|'expediente'|
     'crimen')`), no más filas de texto. La lista de la derecha además
     dejó de estar numerada ("1. 2. 3." → texto plano), lo que NO afecta
     las coordenadas (siguen siendo por posición Y, no por el texto).
  5. El overlay de DebugScene NO bloquea los clicks hacia la escena de
...
from frame_coords import action_menu_item, dialogue_option, dialogue_skip_zone, icon_toolbar_item
...
def run():
    ...
    with sync_playwright() as p:
        ...
        # LocationScene(terminal_sur): items=[Simón(0),Beba(1),Explorar(2),4 viajar(3-6)]
        click(*icon_toolbar_item('expediente'))
        page.wait_for_timeout(300)
        ...
        # Pizarrón: items=[Simón(0),Beba(1),Explorar(2),4 viajar(3-6)] -> ícono, no lista
        click(*icon_toolbar_item('pizarron'))
        page.wait_for_timeout(400)
        ...
        # LocationScene(oeste_profundo): items=[Cacho(0),Explorar(1),5 viajar(2-6)] -> Pizarrón = ícono
        click(*icon_toolbar_item('pizarron'))
        page.wait_for_timeout(400)
        ...
        # LocationScene(parque_obrero): items=[Pipo(0),Palomas(1),Explorar(2),4 viajar(3-6)]
        click(*icon_toolbar_item('crimen'))  # Sistema de Inteligencia Criminal
        page.wait_for_timeout(500)
